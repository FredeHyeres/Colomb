"""seed_demo_nl.py — Demoduivenmelkerij (NL), optioneel te laden
30 duiven 3 generaties, 30 wedvluchten, voeding, trainingen, gezondheid, stambomen
Gebruik: docker exec colombo_backend python seed_demo_nl.py
"""
import asyncio
import asyncpg
import uuid
...
async def main():
    ...
    print("✅ Liefhebber ingevoegd")

    # ══════════════════════════════════════════════════════
    # STAP 4 — LIJNEN
    # ══════════════════════════════════════════════════════
    lignees_data = [
        ("Janssen",       "België",      "#1565C0"),
        ("Van Loon",      "Nederland",   "#2E7D32"),
        ("Vandenabeele",  "België",      "#6A1B9A"),
        ("Geerinckx",     "België",      "#E65100"),
        ("Delbar",        "Frankrijk",   "#AD1457"),
        ("Lokale PACA",   "Frankrijk-PACA", "#00838F"),
    ]
    L = {}
    for nom, origine, couleur in lignees_data:
        ...
        L[nom] = lid
    print(f"✅ {len(L)} lijnen ingevoegd")

    # ══════════════════════════════════════════════════════
    # STAP 5 — DUIVEN  (key, ringnummer, jaar, geslacht, status, lijn, vader_key, moeder_key, naam)
    # ══════════════════════════════════════════════════════
    G1 = [
    ...
    print(f"✅ {len(P)} duiven ingevoegd")

    # ══════════════════════════════════════════════════════
    # STAP 6 — KOPPELS & NESTEN
    # ══════════════════════════════════════════════════════
    couples_def = [
        ("C1","P_G101","P_G102",2021,"A1",[
            (date(2021,2,10),date(2021,3,5),  2,"→ Champion, Diamant"),
            (date(2022,3,15),date(2022,4,8),  2,"→ 2 jongen verkocht"),
        ]),
    ...
    print(f"✅ 5 koppels, {nb_n} nesten ingevoegd")
    ...
    for pk in vaccin_targets:
        sante_rows.append((P[pk],date(2026,1,15),"vaccination","Paramyxovirus","Colombovac PMV-vaccin"))
        sante_rows.append((P[pk],date(2026,2,1), "vaccination","Salmonella",   "Salmonella-vaccin"))
    for pk in g2_repro:
        sante_rows.append((P[pk],date(2026,1,10),"visite_veterinaire","Jaarlijkse controle voor het seizoen. Niets bijzonders.",None))

    sante_rows += [
        (P["P_G305"],date(2026,3,8), "visite_veterinaire","Onderzoek na trichomonase. Behandeling voorgeschreven.",None),
        (P["P_G305"],date(2026,3,9), "observation","Snavel licht vervuild vóór behandeling",None),
        (P["P_G305"],date(2026,3,10),"traitement","Trichomonase — ronidazol 7 dagen","Ronidazol"),
        (P["P_G311"],date(2026,5,1), "traitement","Coccidiose — toltrazuril 3 dagen","Toltrazuril"),
        (P["P_G311"],date(2026,5,20),"traitement","Ornithose — doxycycline 10 dagen","Doxycycline"),
        (P["P_G311"],date(2026,5,25),"visite_veterinaire","Kritieke toestand. Voorbehoud bij de prognose.",None),
        (P["P_G311"],date(2026,5,28),"observation","Algemene toestand sterk verslechterd — overleden op 02/06/2026",None),
        (P["P_G206"],date(2025,8,15),"traitement","Rondwormen — fenbendazol","Fenbendazol"),
        (P["P_G206"],date(2025,9,1), "observation","Met pensioen gestuurd wegens chronische vermoeidheid",None),
        (P["P_G303"],date(2026,7,9), "observation","Lichte piepende ademhaling, preventief behandeld",None),
        (P["P_G303"],date(2026,7,10),"traitement","Mycoplasma — tylosine 7 dagen","Tylosine"),
    ]
    ...
    print(f"✅ {len(sante_rows)} gezondheidsregistraties ingevoegd")

    # ══════════════════════════════════════════════════════
    # STAP 8 — PRESTATIES (30 wedvluchten)
    # ══════════════════════════════════════════════════════
    CONCOURS = [
        ...
        {"c":"C28","d":date(2027,5,18), "l":"Nîmes",        "km":220,"n":275,
         "r":[("P_G301",5,1501),("P_G303",33,1412),("P_G307",91,1334)]},  # P_G306 verloren — afwezig
    ...
    for c in CONCOURS:
        ...
        for pk, rang, vit in c["r"]:
            ...
            perf_count += 1
    print(f"✅ {len(CONCOURS)} wedvluchten, {perf_count} prestaties ingevoegd")

    # ══════════════════════════════════════════════════════
    # STAP 9 — TRAININGEN
    # ══════════════════════════════════════════════════════
    SESSIONS = [
        (date(2026,3,1),  "toss",50, "mooi weer",    16.0,10,None),
        (date(2026,3,8),  "toss",60, "matige wind",  14.0,20,"N"),
        (date(2026,3,15), "toss",65, "mooi weer",    15.0,15,None),
        (date(2026,3,22), "toss",70, "warm",         18.0,5, "S"),
        (date(2026,4,5),  "toss",75, "mooi weer",    17.0,8, None),
        (date(2026,4,12), "toss",80, "mistig",       12.0,10,"W"),
        (date(2026,4,19), "toss",85, "mooi weer",    16.0,12,None),
        (date(2026,4,26), "toss",90, "matige wind",  15.0,25,"NW"),
        (date(2026,5,3),  "toss",95, "warm",         19.0,3, None),
        (date(2026,5,10), "toss",85, "mooi weer",    17.0,10,None),
    ]
    # Scores (recovery, motivation, condition, hydration) per sessie, 0-10
    SCORES = {
    ...
    for pk in concours_pigs:
        await conn.execute("""
            ...
            VALUES ($1,$2,$3,$4,$5,$6)
        """, P[pk], PLAN["Halve fond seizoen"], date(2026,3,1), date(2026,6,30), False, "wedvlucht")
    for pk in actif_pigs:
        await conn.execute("""
            INSERT INTO nutrition_assignments
              (pigeon_id, plan_id, date_debut, date_fin, is_individual, groupe)
            VALUES ($1,$2,$3,$4,$5,$6)
        """, P[pk], PLAN["Rustperiode"], date(2026,1,1), date(2026,2,28), False, "actief")

    aff_n = await conn.fetchval("SELECT COUNT(*) FROM nutrition_assignments")
    print(f"✅ {aff_n} voedingstoewijzingen ingevoegd")
